...
/**
 * La cola de trabajo clínico: consultas abiertas que esperan, citas de hoy y
 * pacientes internados.
 *
 * Nace de un camino que existía pero era a ciegas: recepción abre la consulta
 * desde la cita, y hasta entonces el borrador quedaba colgado del paciente sin
 * ninguna pantalla que los enseñara juntos. Había que acordarse de a quién le
 * habían abierto algo y buscarlo en `/pacientes`.
 *
 ...
 * avisos a los clientes y cada uno descuenta del cupo del plan; esto es trabajo
 * clínico. Que el cupo se gaste por un solo lado es la razón de que los botones
 * de aviso no estén aquí.
 *
 * Lo usan dos pantallas. Sin `veterinarioId` enseña la clínica entera, que es
 * lo que necesita el administrador: en el Plan Consultorio atiende él y no hay
 * nadie más, y en una clínica con varios veterinarios coordina. De ese único
 * dato se derivan los rótulos y la columna del veterinario, para que no puedan
 * quedar descuadrados respecto a lo que se está pidiendo.
 */
export function JornadaClinica({
  veterinarioId,
  sucursalId,
}: {
  ...
  /** Acotada a una persona: los rótulos hablan de tú y sobra decir de quién es. */
  const propia = Boolean(veterinarioId)
  ...
  const recargar = useCallback(async () => {
    ...
    const [abiertas, citas, ingresados] = await Promise.all([
      listConsultasAbiertas(veterinarioId, sucursalId),
      listCitas(sucursalId, rango, veterinarioId),
      listInternaciones(sucursalId, 'internado', veterinarioId),
    ])

    setConsultas(abiertas)
    setCitasHoy(citas)
    setInternados(ingresados)
  }, [veterinarioId, sucursalId])
  ...
  /** Columna que solo aparece cuando se está mirando a más de una persona. */
  function columnaVeterinario<T extends { veterinario_nombre: string }>(): Columna<T>[] {
    if (propia) return []
    return [
      {
        clave: 'veterinario',
        cabecera: 'Veterinario',
        celda: (f) => <span className="text-slate-600">{f.veterinario_nombre}</span>,
      },
    ]
  }
  ...
  const columnasConsultas: Columna<ConsultaAbierta>[] = [
    ...
    ...columnaVeterinario<ConsultaAbierta>(),
    {
      clave: 'cuando',
      cabecera: 'Citado',
      celda: (c) =>
  ...
  const columnasCitas: Columna<CitaConDetalle>[] = [
    ...
    ...columnaVeterinario<CitaConDetalle>(),
    {
      clave: 'tipo',
      cabecera: 'Tipo',
      celda: (c) => (
  ...
  const columnasInternados: Columna<InternacionConDetalle>[] = [
    ...
    ...columnaVeterinario<InternacionConDetalle>(),
    {
      clave: 'evolucion',
  ...
  return (
    <div className="space-y-6">
            ...
            vacio={
              <span className="flex flex-col items-center gap-2 text-slate-400">
                ...
                {cargando
                  ? 'Cargando…'
                  : propia
                    ? 'No hay consultas abiertas esperándote.'
                    : 'No hay ninguna consulta abierta sin cerrar.'}
              </span>
            }
          />
        </div>
      ...
      <Seccion titulo={propia ? 'Tus citas de hoy' : 'Citas de hoy'} icono={<CalendarDays size={14} />}>
        <div className="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
          <TablaResponsive
      ...
      <Seccion
        titulo={propia ? 'Internados a tu cargo' : 'Pacientes internados'}
            ...
            vacio={
              <span className="flex flex-col items-center gap-2 text-slate-400">
                ...
                {cargando
                  ? 'Cargando…'
                  : propia
                    ? 'No tienes pacientes internados.'
