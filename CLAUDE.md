...
- **`clinicas_con_catalogo()`** es una función `security definer`, mismo patrón que `clinicas_para_registro()` (registro público, §Sesión y acceso): `clinicas_select` (`id = auth_clinica_id() or auth_es_plataforma()`) no deja leer la fila de otra clínica, ni siquiera incrustada en un `select('*, clinicas(...)')`. La función expone solo las columnas seguras (`nombre`, `logo_url`, `ciudad`, `tipo_negocio`, `whatsapp`) — nunca `responsable`, cuota de WhatsApp, estado de pago ni plan contratado. `grant execute` va a `authenticated`, no a `anon`: la Tienda solo se ve con sesión iniciada en el portal. ⚠️ **Esa última frase describe la intención, no lo que hay desplegado.** Comprobado contra producción: `clinicas_con_catalogo` responde **HTTP 200 con datos** a la clave anónima sin sesión, y lo mismo `clinicas_con_peluqueria` y `servicios_peluqueria_de` (vacías hoy, pero alcanzables). El motivo es que Supabase concede `execute` a `anon`, `authenticated` y `service_role` **por defecto** sobre toda función nueva de `public`, y el `revoke all … from public` de las migraciones quita el pseudo-rol `PUBLIC` pero **no ese grant explícito a `anon`**. Para cerrarlo hace falta `revoke execute on function … from anon`. Mientras tanto, lo que se expone sin sesión es el nombre, logo, ciudad y **WhatsApp** de las clínicas con el módulo.
