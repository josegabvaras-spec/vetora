...
- [supabase/migrations/](supabase/migrations/) es el esquema de verdad y **es normativo** — pero eso son **más de setenta** migraciones (`0001` a `0077` a la fecha, y sigue creciendo — no cites el número exacto de memoria, cuéntalo con `ls supabase/migrations/*.sql | wc -l`), no solo la primera. `0001_init.sql` es la base tal como quedó el día que se escribió (20 tablas, 35 policies, 3 triggers, 4 funciones `SECURITY DEFINER`) — hoy el esquema tiene **46** tablas (recontadas el 2026-09-12; decía 44 desde antes de `intentos_publicos` en `0068` y `registro_respaldos` en `0074`, y nadie volvió a contar) y una quinta función `auth_*` (`auth_es_personal()`, añadida en `0004`). De `0002` en adelante hay correcciones de seguridad reales y features aplicadas encima, hasta la más reciente (RLS de `historial_update`/`internaciones_update`, índices, cuota de WhatsApp, portal del cliente, `pacientes.codigo`/`foto`, venta directa, recetario, tipo de cita `peluqueria`, inventario fraccionado, el respaldo completo de 37 tablas, el tope de gasto del copiloto, el MFA del superadmin…). Leer solo `0001` da una foto vieja del esquema — antes de decir "esta tabla/columna no existe" o "esta policy no tiene `with check`", revisa si una migración posterior ya lo tocó.
...
- ~~`lib/exportacion.ts` e `lib/importacion.ts` leen y escriben tablas enteras desde `lib/`~~ — **corregido.** `generarRespaldo()` era la única función de las dos que consultaba Supabase, y encima la importaba **directamente `RespaldoPage`**, saltándose la capa de servicios. Vive ahora en [services/respaldo.ts](src/services/respaldo.ts); los dos `lib/` quedaron puros (CSV, ZIP y descarga), que es lo que comparten los **dos** caminos del respaldo — el de la clínica y el de la plataforma ([services/respaldoPlataforma.ts](src/services/respaldoPlataforma.ts), que pide las filas a la Edge Function con `service_role`). Si vuelves a meter una consulta en esos `lib/`, esa separación se pierde. Los `any` que quedan son de las filas del CSV, donde la forma es dinámica de verdad y todo acaba siendo texto.
...
- **`DatosProducto` ignoraba cinco columnas que `0030` añadió para todos**: `costo_bs`, `codigo_barras`, `marca`, `proveedor_id` y `requiere_lote`. Sin el costo la clínica no podía ver su margen en ninguna pantalla; sin el código de barras no había nada que escanear. Todas opcionales — el alta rápida de un fármaco sigue siendo nombre, precio y presentación. **El costo es interno y no viaja a la vitrina del portal** (`publicarProductoEnTienda` copia solo el precio de venta), y `productos_all` tampoco lo deja leer a un cliente —exige `auth_es_admin()` o coincidir de sucursal, y una cuenta de portal no tiene ninguna—. ⚠️ **Esto describía el estado de `0030`, y ya no es el actual — corregido en `0045` y confirmado contra producción, no lo repitas como hallazgo abierto.** `producto_lotes.costo_unitario_bs`, `proveedores`, `ordenes_compra`, `orden_compra_detalles`, `petshop_devoluciones`, `petshop_promociones` y `petshop_configuracion` nacieron en `0030` con `using (clinica_id = auth_clinica_id())` para `authenticated`, **sin `auth_es_personal()`** — un cliente del portal, que también tiene `clinica_id`, podía leer las siete. `0045_permisos_escritura_lectura.sql` reescribió las siete policies de lectura añadiendo `and auth_es_personal()`, y el retest del 2026-09-08 lo verificó en vivo: «Policies de las 7 tablas de `0030` sin rol → **0 filas**» (`SEGURIDAD.md`, tabla de evidencia del retest). Un barrido de las **207** `create policy` de todo `supabase/migrations/` (auditoría OWASP de 2026-09-12, ver `AUDITORIA_OWASP_VETORA.md`) confirma que el patrón no se repite en ninguna tabla que no sea explícitamente del portal (`_portal` en el nombre) o pública por diseño (`planes_select`, `catalogo_productos_portal`). La lección de este propio párrafo quedándose desactualizado: un hallazgo cerrado en el código sigue abierto en la documentación hasta que alguien corrige el texto — el mismo defecto que ya costó el incidente de H-15 y el de las migraciones «NO APLICADA TODAVÍA» de `SEGURIDAD.md`.
...
⚠️ **Esas invariantes se vencían borrando al PADRE, por DOS vías — hoy solo queda una abierta.** `pacientes_personal` es `for all` para todo el personal, y desde `pacientes` cascadean doce tablas. **Las cascadas de clave foránea no evalúan la RLS ni disparan los triggers `before update`** (el de `historial_clinico` es justo ese tipo), así que un `DELETE` podía llevarse por delante el expediente sin que nada lo notara. Esto era H-31 del retest del 2026-09-08, y tenía dos puertas:

- **Borrar el paciente directamente.** `trg_paciente_sin_caja` (`before delete` en `pacientes`, 0049) ya bloqueaba si había cobros; `0076` lo amplió para bloquear también si hay **historiales cerrados** (`editable = false`) — un borrador abierto no cuenta, es trabajo a medias, no el expediente que se promete inmutable.
- **Borrar la CITA, no el paciente.** `historial_clinico.cita_id` es `on delete cascade`, y `citas_personal` era `for all` con DELETE incluido — borrar una cita es rutinario, no raro como borrar un paciente. `0075` le quita el DELETE (la app ya cancela con `actualizarEstadoCita`, nunca borra).
...
- **Moneda: dos, y no se mezclan.** Todo lo de la clínica —consultas, productos, caja, recibos— va en bolivianos con `formatBs()` de [lib/currency.ts](src/lib/currency.ts) (`Bs. 0.00`): quien paga ahí es un dueño de mascota en Bolivia.
  ...
  **No tiene ruta propia ni entrada de menú, a propósito**: pagar la suscripción es una gestión de la cuenta, no del trabajo del día, y en el menú lateral quedaba mezclada con la agenda y el inventario. La pestaña solo se dibuja para el `admin` — `PerfilModal` sigue siendo el modal estrecho de siempre para el veterinario y para recepción.
  ...
  Una clínica **suspendida no puede entrar y por tanto no puede pagar desde aquí**: eso es a propósito y se resuelve hablando con el dueño de la plataforma. Facturación sirve al estado **`en_mora`**, que es el previo. Dejar entrar a un admin suspendido convertiría la suspensión en fachada. ⚠️ **Esa frase decía «porque hoy no está en la RLS: su JWT seguiría leyendo su clínica entera desde PostgREST», y eso lo cerró `0067`** (VUL-24), que lleva el estado de la clínica a `auth_es_personal()`/`auth_es_admin()`/`auth_es_clinico()`/`auth_ve_expediente()`. La suspensión ya no es solo de interfaz; ver «Sesión y acceso» más arriba.
...
- **Roles:** ocultar un enlace en el `Sidebar` no basta; la ruta va envuelta en `RolRoute` en [App.tsx](src/App.tsx). Ambas cosas. El reparto actual:
  ...
  **El peluquero da de alta mascotas y dueños, pero no ve el expediente clínico.** Al principio se le negó `/pacientes` entera, porque `FichaPacientePage` no distinguía quién escribe. Eso dejaba a una peluquería sin poder registrar a su propio paciente —y sin paciente no hay a quién agendarle, ni nada que enseñarle al dueño en el portal—, cuando es exactamente el mismo alta que hace una clínica. Ahora entra a `/pacientes` y `/clientes`, y lo que se acota es el expediente, en dos sitios que van emparejados: `puedeVerHistorialClinico()` ([lib/personal.ts](src/lib/personal.ts)) le oculta las tres pestañas (historial, esquema sanitario, internaciones) y el menú de impresión, y el `RolRoute` de las rutas de impresión cierra la puerta de atrás — esas seis colgaban solo de `ProtectedRoute` y se abrían tecleando la URL. Si añades una pantalla clínica, decide en cuál de las dos listas va.

  ⚠️ **Y desde `0053` la RLS sí te frena, que antes no.** `auth_es_personal()` incluye al peluquero desde `0025`, así que las policies del expediente colgaban de una función que lo dejaba pasar: por PostgREST podía leer un historial, escribirlo, **cerrarlo** (el `with check` de `historial_update` no exige `editable`, así que el UPDATE que lo cierra pasaba) y recetar, todo lo que la interfaz le esconde. `0053` mueve las **27 policies** de las nueve tablas del expediente —y las tres del bucket `estudios`— a **`auth_ve_expediente()`**: admin, veterinario y recepción, con `activo`.

  **Es el espejo exacto de `puedeVerHistorialClinico()`, y esa es la regla: si cambias una, cambia la otra.** No sirve `auth_es_clinico()` (0042, admin + veterinario) porque excluiría a recepción, y recepción abre la consulta desde la cita y registra el esquema sanitario. Lo que NO se tocó es lo suyo: `pacientes`, `clientes`, `citas` y las `peluqueria_*` siguen en `auth_es_personal()`.
...
⚠️ **Sigue sin ser una revocación de sesión, pero desde `0050` la RLS SÍ lo aplica.** `alternarActivoUsuario()` no revoca el JWT ni toca `auth.users`: el empleado desactivado conserva su token y lo renueva con su refresh token. Lo que cambió es que ese token ya no le sirve de nada — `0050` reescribió `auth_clinica_id()`, `auth_es_admin()`, `auth_es_personal()` y `auth_es_plataforma()` para exigir `and activo`, así que un usuario con `activo = false` obtiene `null`/`false` de las cuatro y **las policies que cuelgan de ellas le devuelven cero filas por PostgREST**. (`auth_sucursal_id()` no se tocó a propósito: devuelve una sucursal, no un permiso.) Verificado aplicado en producción el 2026-09-08.

⚠️ **Este párrafo dijo lo contrario durante un tiempo** —«un empleado desactivado sigue leyendo y escribiendo las 44 tablas»— porque `0050` se aplicó y su cabecera siguió diciendo «NO APLICADA TODAVÍA». Si aplicas una migración, corrige su cabecera **y** lo que este archivo afirme sobre ella.

**La otra mitad, la suspensión de la clínica, la cierra `0067`** (VUL-24), que añade `join clinicas … and c.estado <> 'suspendida'` a `auth_es_personal()`, `auth_es_admin()`, `auth_es_clinico()` y `auth_ve_expediente()` — deliberadamente **no** a `auth_clinica_id()`, porque eso repetiría la regresión H-15 (`motivoDeBloqueo()` corre en cada login y necesita poder leer la clínica para explicar por qué está bloqueada). Verificado aplicado el 2026-09-08.

⚠️ **Estas funciones se redefinen en varias migraciones y `create or replace` sobrescribe: re-ejecutar una migración vieja revierte las nuevas SIN ningún error.** Pasó de verdad el 2026-09-08 — re-correr `0050` le quitó la suspensión a `auth_es_personal`/`auth_es_admin` y **el MFA a `auth_es_plataforma`**, dejando además el esquema a medias (`auth_es_clinico` y `auth_ve_expediente` conservaron su versión buena porque `0050` no las toca). Lo reparó `0073`. El orden vigente es **`0050` (activo) → `0067` (suspensión) → `0072` (MFA) → `0073` (repara)**: antes de re-ejecutar cualquiera de ellas, mira qué migración posterior toca el mismo objeto. Para ver el estado real: `select proname, prosrc from pg_proc where proname like 'auth\_%'`.
...
- **`/respaldo`**: la lectura de las 37 tablas vive en [services/respaldo.ts](src/services/respaldo.ts); el armado del ZIP (JSZip + file-saver) y el CSV, en [lib/exportacion.ts](src/lib/exportacion.ts) / [lib/importacion.ts](src/lib/importacion.ts), que son **puros a propósito** — los comparten el respaldo de la clínica y el de la plataforma. Un CSV por tabla y una carpeta `fotos/` con las de los pacientes indexadas por `codigo` (fuera del CSV, que solo lleva `tiene_foto`). La importación fusiona por `id` sobre lo existente, no reemplaza. El CSV es plano y sin escapes fuertes: no lo trates como un formato de intercambio estable.
  ⚠️ **Las firmas manuscritas (`consentimientos_cirugia`/`informes_firmados`, ver H-32 en [SEGURIDAD.md](SEGURIDAD.md)) llevaban meses saliendo íntegras en el CSV** — la misma imagen en base64 que ya se le quitó a la foto del paciente desde el principio, sin que nadie replicara el mismo tratamiento. Ahora reciben exactamente el mismo patrón: se sacan de la fila (el CSV solo lleva `tiene_firma`) y viajan en una carpeta `firmas/` propia, indexadas por `{tabla}_{id}_{campo}.png`, con `leerZip()` reincorporándolas byte a byte al restaurar. Si añades otra columna de imagen pesada a una tabla del respaldo, este es el patrón a copiar — no la excepción de `pacientes.foto`.
  ⚠️ **Las 37 tablas se leían con `select('*')` suelto, sin paginar (H-34 en `SEGURIDAD.md`).** PostgREST corta en 1000 filas sirviendo un `200 OK` normal, sin error: una clínica con más de mil citas o mil líneas de historial se descargaba un ZIP con esa tabla truncada, sin ninguna señal. Ahora `services/respaldo.ts` usa `traerTodo()` ([lib/paginacion.ts](src/lib/paginacion.ts)), y la Edge Function `respaldo-clinica` lleva su propia copia del mismo patrón (`traerTablaCompleta`, no puede importar de `src/` porque corre en Deno). Si añades una tabla nueva al respaldo, va por ahí, nunca por un `select('*')` a secas.
- **`/metricas`** ([services/metricas.ts](src/services/metricas.ts)): comparativas mes actual contra mes anterior; los gráficos son `recharts`. Todo el servicio —incluido el bucle que arma el historial de los últimos N meses— compara meses con `clinicMonth()` de `lib/datetime`, en la zona de la clínica. ⚠️ **Este párrafo decía que ese bucle seguía usando `getMonth()` del navegador, y era falso**: la migración ya estaba hecha y las menciones a `getMonth()` que quedan en el fichero son comentarios explicando el fallo antiguo (el gráfico y las tarjetas de la misma pantalla se contradecían con un cobro del día 31 por la noche). Si vas a "terminar de migrarlo", léelo primero.
...
**La CSP está en modo BLOQUEO** (`Content-Security-Policy`, sin el sufijo `-Report-Only`), y por tanto **sí protege**: el navegador impide la carga que viole la política en vez de solo anotarla. Verificado en producción con `curl -I https://www.vetora.online`.

⚠️ **Esto cambió, y durante un tiempo este párrafo dijo lo contrario.** Nació en `-Report-Only` —que no bloquea nada y es solo un instrumento de medición— porque una CSP mal escrita deja la aplicación en blanco, y subirla a bloqueo exigía recorrer antes la aplicación comprobando que no reporta violaciones. Ese recorrido se hizo (ver H-14 y, sobre todo, **VUL-14 «La CSP pasa de medir a bloquear»** en [SEGURIDAD.md](SEGURIDAD.md), que es la entrada vigente) y se subió a bloqueo. Si tocas los orígenes de la política, recuerda que ahora un fallo **rompe la pantalla** en vez de anotarse en consola: revertir es cambiar una palabra en `vercel.json`.

⚠️ **Lo que sigue sin recorrerse con `enforce` es el área autenticada del PERSONAL** (el portal del cliente sí se recorrió entero, con 0 violaciones). Es donde viven el realtime, las fotos de paciente y las URL firmadas de Storage — los tres orígenes ya están en la política, pero comprobado no está.
