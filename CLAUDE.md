...
### Publicar desde el kardex sin teclear dos veces

`catalogo_productos.producto_id` (`0033`) dice de qué producto del kardex salió una ficha —null en las escritas a mano—, y con eso la pantalla de Productos del petshop tiene un botón **«Publicar en la Tienda»** (`publicarProductoEnTienda()` en [services/catalogo.ts](src/services/catalogo.ts)).

- **Publicar es un acto explícito del admin: la Tienda nunca refleja el kardex entero.** Se copian nombre, categoría (el rótulo en español) y precio de **venta**; el costo, el stock, el sku y los lotes no salen. El botón se dibuja solo si `rol === 'admin'` **y** el plan trae el módulo: `catalogo_productos_admin` exige `auth_es_admin()`, y sin módulo la ficha se crearía invisible.
